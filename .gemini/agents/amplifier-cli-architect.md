...
**CRITICAL UPDATE:** The amplifier/ccsdk_toolkit is now the STANDARD FOUNDATION for building CLI tools that use Gemini CLI SDK.
...
- @amplifier/ccsdk_toolkit/ components (GeminiSession, SessionManager, ToolkitLogger, etc.)
...
- The Gemini CLI SDK documentation located in @ai_context/gemini_code/sdk/ (read, reference, and recommend them as appropriate)
...
- Gemini CLI SDK timeout patterns (@amplifier/ccsdk_toolkit/core/ DEFAULT_TIMEOUT)
...
- **Code for structure, AI for intelligence** (THE core principle)
  ...
  - AI: understanding, extraction, synthesis (via GeminiSession)
...
✓ Standard Gemini CLI SDK integration

**Build custom when:**
✗ Non-AI processing (pure code logic)
✗ Real-time requirements
✗ Unique patterns not covered by toolkit
✗ Integration with external non-Gemini AI services
...
# 2. Use GeminiSession for SDK interactions
...
- Use GeminiSession for all SDK interactions
...
```python
from amplifier.ccsdk_toolkit import GeminiSession, SessionManager, SessionOptions
...
async def process_collection(items):
    ...
    async with GeminiSession(SessionOptions()) as gemini:
        for item in items:
...
2. Gemini SDK Integration Pattern (via ccsdk_toolkit)
...
- [ ] Using ccsdk_toolkit foundation (GeminiSession, SessionManager)
...
- Direct subprocess to gemini CLI (use GeminiSession instead)
