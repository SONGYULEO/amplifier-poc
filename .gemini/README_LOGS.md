# Gemini CLI Hook Logs

The `.gemini/logs` directory contains file-based logs from Gemini CLI hooks for debugging and monitoring.
...
```bash
tail -f .gemini/logs/stop_hook_*.log
```

To search for errors:

```bash
grep ERROR .gemini/logs/*.log
```

To see today's logs:

```bash
ls -la .gemini/logs/*_$(date +%Y%m%d).log
```

## Implementation

The logging is implemented in `.gemini/tools/hook_logger.py` which provides:
