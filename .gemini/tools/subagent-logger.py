...
def ensure_log_directory() -> Path:
    ...
    # Use .gemini/logs directory for consistency with other Gemini CLI hooks
    project_root = Path(os.environ.get("GEMINI_PROJECT_DIR", os.getcwd()))
    log_dir = project_root / ".gemini" / "logs" / "subagent-logs"
...
def main() -> NoReturn:
    try:
        ...
        # Silently fail to not disrupt Gemini's workflow
    ...
    if data.get("hook_event_name") == "PreToolUse" and data.get("tool_name") == "Task":
        try:
            ...
            # Log error but don't block Gemini's operation
            print(f"Warning: Failed to log subagent usage: {e}", file=sys.stderr)

    # Always exit successfully to not block Gemini's workflow
    sys.exit(0)
