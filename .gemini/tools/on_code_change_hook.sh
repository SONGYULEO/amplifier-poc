...
# Gemini CLI make check hook script
...
# Check if we're editing a file in the gemini project dir
# GEMINI_PROJECT_DIR is provided by Gemini CLI and points to the project root
if [[ -n "${FILE_PATH:-}" ]] && [[ -n "${GEMINI_PROJECT_DIR:-}" ]] && [[ "$FILE_PATH" == "$GEMINI_PROJECT_DIR"/* ]]; then
    # Editing a file in the gemini project dir project - always run from project root
    START_DIR="$GEMINI_PROJECT_DIR"
    echo "Detected edit in project - will run checks from: $GEMINI_PROJECT_DIR"
else
    # Determine the starting directory for non-gemini project dir files
