...
> Understanding the two input modes for Gemini CLI SDK and when to use each

## Overview

The Gemini CLI SDK supports two distinct input modes for interacting with agents:
...
Streaming input mode is the **preferred** way to use the Gemini CLI SDK. It provides full access to the agent's capabilities and enables rich, interactive experiences.
...
```python Python
from gemini_code_sdk import GeminiSDKClient, GeminiCodeOptions, AssistantMessage, TextBlock
import asyncio
import base64
...
async def streaming_analysis():
    ...
    # Use GeminiSDKClient for streaming input
    options = GeminiCodeOptions(
        max_turns=10,
        allowed_tools=["Read", "Grep"]
    )

    async with GeminiSDKClient(options) as client:
...
```python Python
from gemini_code_sdk import query, GeminiCodeOptions, ResultMessage
import asyncio
