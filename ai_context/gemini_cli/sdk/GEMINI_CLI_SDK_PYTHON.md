...
> Build custom AI agents with the Gemini CLI Python SDK

## Prerequisites

- Python 3.10+
- `gemini-code-sdk` from PyPI
- Node.js 18+
- `@anthropic-ai/gemini-code` from NPM

<Note>
  To view the Python SDK source code, see the [`gemini-code-sdk`](https://github.com/anthropics/gemini-code-sdk-python) repo.
</Note>
...
Install `gemini-code-sdk` from PyPI and `@anthropic-ai/gemini-code` from NPM:

```bash
pip install gemini-code-sdk
npm install -g @anthropic-ai/gemini-code  # Required dependency
```
...
# legal-agent.py
import asyncio
from gemini_code_sdk import GeminiSDKClient, GeminiCodeOptions
...
### 1. The `GeminiSDKClient` class (recommended)
...
async for message in query(
    ...
    options=GeminiCodeOptions(system_prompt="You are a performance engineer")
):
...
The Python SDK accepts all arguments supported by the [command line](/en/docs/gemini-code/cli-reference) through the `GeminiCodeOptions` class.

### GeminiCodeOptions parameters
...
- **`model`**: `str | None` - Specific Gemini model to use
- **`max_thinking_tokens`**: `int` - Maximum tokens for Gemini's thinking process (default: 8000)
...
- **`extra_args`**: `dict[str, str | None]` - Pass arbitrary CLI flags to underlying Gemini CLI CLI
...
async with GeminiSDKClient() as client:
    ...
    # Message with image reference (image will be read by Gemini's Read tool)
...
# The SDK handles image files through Gemini's built-in Read tool
# Supported formats: PNG, JPG, PDF, and other common formats
```

## Multi-turn conversations

### Method 1: Using GeminiSDKClient for persistent conversations

```python
import asyncio
from gemini_code_sdk import GeminiSDKClient, GeminiCodeOptions, query
...
async def resume_session():
    ...
    async for message in query(
        ...
        options=GeminiCodeOptions(continue_conversation=True)
    ):
        if type(message).__name__ == "ResultMessage":
            print(message.result)
...
async def process_inputs():
    async with GeminiSDKClient() as client:
        ...
        # Image input (Gemini will use Read tool automatically)
...
async with GeminiSDKClient() as reviewer, GeminiSDKClient() as tester:
...
# Error handling
from gemini_code_sdk import CLINotFoundError, ProcessError
...
try:
    ...
    print("Install CLI: npm install -g @anthropic-ai/gemini-code")
...
- [GitHub Actions integration](/en/docs/gemini-code/github-actions) - Automate your GitHub workflow with Gemini
