...
> Control tool usage and permissions in the Gemini CLI SDK
...
The Gemini CLI SDK provides powerful permission controls that allow you to manage how Gemini uses tools in your application.

This guide covers how to implement permission systems using the `canUseTool` callback, hooks, and settings.json permission rules. For complete API documentation, see the [TypeScript SDK reference](/en/docs/gemini-code/typescript-sdk-reference).

## Overview

The Gemini CLI SDK provides four complementary ways to control tool usage:

1. **[Permission Modes](#permission-modes)** - Global permission behavior settings that affect all tools
2. **[canUseTool callback](/en/docs/gemini-code/typescript-sdk-reference#canusetool)** - Runtime permission handler for cases not covered by other rules
3. **[Hooks](/en/docs/gemini-code/typescript-sdk-reference#hook-types)** - Fine-grained control over every tool execution with custom logic
4. **[Permission rules (settings.json)](/en/docs/gemini-code/settings#permission-settings)** - Declarative allow/deny rules with integrated bash command parsing
...
Permission modes provide global control over how Gemini uses tools. You can set the permission mode when calling `query()` or change it dynamically during streaming sessions.
...
| `plan`              | Planning mode - no execution | Gemini can only use read-only tools; presents a plan before execution **(Not currently supported in SDK)** |
...
- Speeds up development when you trust Gemini's edits
...
- **Use with extreme caution** - Gemini has full system access
...
canUseTool fires whenever Gemini CLI would show a permission prompt to a user, e.g. hooks and permission rules do not cover it and it is not in autoaccept mode.
...
```python Python
from gemini_code_sdk import query, GeminiCodeOptions, HookMatcher, HookContext
...
Permission rules in `settings.json` provide declarative control with built-in bash command parsing. These rules are evaluated before `canUseTool` is called. For more details on settings configuration, see the [Gemini CLI settings documentation](/en/docs/gemini-code/settings).
...
2. **Use permission rules** for static policies, especially bash commands (see [permission settings](/en/docs/gemini-code/settings#permission-settings))
3. **Use hooks** to log, audit, or transform all tool uses (see [hook types](/en/docs/gemini-code/typescript-sdk-reference#hook-types))
4. **Use canUseTool** for dynamic decisions on uncovered cases (see [CanUseTool type](/en/docs/gemini-code/typescript-sdk-reference#canusetool))
