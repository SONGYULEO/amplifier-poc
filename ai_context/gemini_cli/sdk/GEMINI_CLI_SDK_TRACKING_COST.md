...
> Understand and track token usage for billing in the Gemini CLI SDK

# SDK Cost Tracking

The Gemini CLI SDK provides detailed token usage information for each interaction with Gemini. This guide explains how to properly track costs and understand usage reporting, especially when dealing with parallel tool uses and multi-step conversations.

For complete API documentation, see the [TypeScript SDK reference](/en/docs/gemini-code/typescript-sdk-reference).

## Understanding Token Usage

When Gemini processes requests, it reports token usage at the message level. This usage data is essential for tracking costs and billing users appropriately.

### Key Concepts

1. **Steps**: A step is a single request/response pair between your application and Gemini
...
When Gemini executes tools, the usage reporting differs based on whether tools are executed sequentially or in parallel:
...
**All messages with the same `id` field report identical usage**. When Gemini sends multiple messages in the same turn (e.g., text + tool uses), they share the same message ID and usage data.
...
3. **Report inconsistencies** - File issues at the [Gemini CLI GitHub repository](https://github.com/anthropics/gemini-code/issues)
...
- [TypeScript SDK Reference](/en/docs/gemini-code/typescript-sdk-reference) - Complete API documentation
- [SDK Overview](/en/docs/gemini-code/sdk/sdk-overview) - Getting started with the SDK
- [SDK Permissions](/en/docs/gemini-code/sdk/sdk-permissions) - Managing tool permissions
