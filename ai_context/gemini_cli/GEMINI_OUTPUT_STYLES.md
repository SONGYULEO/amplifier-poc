...
> Adapt Gemini CLI for uses beyond software engineering

Output styles allow you to use Gemini CLI as any type of agent while keeping
its core capabilities, such as running local scripts, reading/writing files, and
tracking TODOs.

## Built-in output styles

Gemini CLI's **Default** output style is the existing system prompt, designed
...
codebase and how Gemini operates:
...
- **Learning**: Collaborative, learn-by-doing mode where Gemini will not only
  share "Insights" while coding, but also ask you to contribute small, strategic
  pieces of code yourself. Gemini CLI will add `TODO(human)` markers in your
  code for you to implement.

## How output styles work

Output styles directly modify Gemini CLI's system prompt.

- Non-default output styles exclude instructions specific to code generation and
  efficient output normally built into Gemini CLI (such as responding concisely
...
These changes apply to the [local project level](/en/docs/gemini-code/settings)
and are saved in `.gemini/settings.local.json`.

## Create a custom output style

To set up a new output style with Gemini's help, run
`/output-style:new I want an output style that ...`

By default, output styles created through `/output-style:new` are saved as
markdown files at the user level in `~/.gemini/output-styles` and can be used
...
the user level (`~/.gemini/output-styles`) or the project level
(`.gemini/output-styles`).

## Comparisons to related features

### Output Styles vs. GEMINI.md vs. --append-system-prompt

Output styles completely “turn off” the parts of Gemini CLI’s default system
prompt specific to software engineering. Neither GEMINI.md nor
`--append-system-prompt` edit Gemini CLI’s default system prompt. GEMINI.md
adds the contents as a user message _following_ Gemini CLI’s default system
prompt. `--append-system-prompt` appends the content to the system prompt.

### Output Styles vs. [Agents](/en/docs/gemini-code/sub-agents)
...
### Output Styles vs. [Custom Slash Commands](/en/docs/gemini-code/slash-commands)
